...
- **SYADOUHUKUIN（車道幅員）**: xlsxが `'12,14'` のように複合コード表記。新コンバーターは `'12'`/`'13'`/`'16'` も個別に定義済み（旧ツールは未定義で空値になっていた）。※これらのコードは2022年データに29,322件出現する（旧ツールとの差異の主因。詳細は [CHECKS.md](CHECKS.md) の②比較結果を参照）
