...
| 年次 | カラム | 旧値 | 新値 | 影響件数（当事者A+B） | 理由 |
|------|--------|------|------|----------------------|------|
| 2022 | 当事者種別（当事者A/B） | `一般原付自転車` | `二輪車－原付自転車` | 約19,561件 | 旧スクリプトのコード誤りを公式xlsxに準拠して修正 |
| 2023 | 当事者種別（当事者A/B） | `一般原付自転車` | `二輪車－原付自転車` | 約19,608件 | 同上 |
| 全年 | エアバッグの装備（当事者A/B） | `非展開` | `その他` | 各年約40〜50万件 | 旧スクリプトのラベル誤り（コード2＝`その他`が公式）。データの実態（エアバッグ非作動）は同一 |

※ 「その他」ラベルの変更は内容に実質的な差異なし。当事者種別の変更は公式コード表に準拠した修正。
